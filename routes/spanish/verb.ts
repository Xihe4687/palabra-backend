...
router.get('/random', async (req, res) => {
  try {
    ...
    const query = `SELECT TOP ${limit} * FROM spanish_verbs ORDER BY -LOG((ABS(CHECKSUM(NEWID())) + 1) / 2147483648.0) / (TRY_CAST(wrongTimes AS float) + 1);`;
...
router.post('/', async (req, res) => {
  try {
    ...
    const rows = await runQuery('select * from spanish_verbs where spanish = ?', [payload.infinity]);
    if (rows.length) {
      res.status(200).json(rows[0]);
    } else {
      res.status(404).json({ error: 'No records found.' });
